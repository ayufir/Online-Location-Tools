...
export default function TeamFleetScreen() {
  ...
  const fetchTeam = async () => {
    try {
      ...
      if (res.data.success && Array.isArray(res.data.data)) {
        // Filter out colleagues without valid locations to prevent crashes
        const validTeammates = res.data.data.filter((t: any) => 
      ...
      console.error('[TeamMap] Fetch Error:', e.message);
      setTeammates([]);
    } finally {
      setRefreshing(false);
      setLoading(false);
  ...
  const focusOnTeam = () => {
    if (Array.isArray(teammates) && teammates.length > 0 && mapRef.current) {
    ...
    <View style={styles.container}>
      ...
      {Platform.OS === 'web' ? (
        ...
        >
          ...
          {(teammates || []).map((emp) => {
              const loc = emp.currentLocation || emp.location;
